...
```
event.recipes.kaleidoscope_cookery.stockpot(output, input[], soup_base, time, carrier, cooking_texture, finished_texture, cooking_bubble_color, finished_bubble_color)
// output: Output item; 
...
event.recipes.kaleidoscope_cookery.stockpot(
    ...
    "minecraft:lava", 600, "minecraft:flower_pot", 
    "kaleidoscope_cookery:stockpot/text_cooking",
    "kaleidoscope_cookery:stockpot/text_finished",
    parseInt("FFFFFF", 16), parseInt("FF0000", 16), 
  )
```

Kaleidoscope Cookery - Pot Recipe:
```
event.recipes.kaleidoscope_cookery.pot(output, input[], carrier, time, cooking_time)
// output: Output item; 
// input[]: Array of input ingredients; 
// carrier: Container for the output item (Optional); 
// time: Cooking time in ticks (Optional); 
